...
import org.jboss.eap.galleon.channels.universe.JBossChannelsRepoManager;
...
public class ChannelMavenArtifactRepositoryManager implements JBossChannelsRepoManager, MavenRepoManager {
    ...
    public ChannelMavenArtifactRepositoryManager(MavenProject project, List<String> urls,
        ...
        for (String s : urls) {
            ...
            try {
                ...
                Path p = resolvePath(project, Paths.get(s));
                if (Files.exists(p)) {
                    url = p.toUri().toURL();
                } else {
                    throw ex;
            ...
            channels.add(ChannelMapper.from(url));
        }
        factory = new WildFlyMavenVersionResolverFactory(null, buildDir, system, contextSession);
        channelSession = new ChannelSession(channels, factory);
    }

    // XXX DEMO, to e replaced with proper support
    @Override
    public String getChannel(String producer) {
        if ("org.jboss.eap:wildfly-ee-galleon-pack".equals(producer)) {
            return "7.0";
        } else {
            if ("org.jboss.eap:wildfly-galleon-pack".equals(producer)) {
                return "4.0";
            }
        }
        return null;
    ...
    public Set<String> getProducers() {
        ...
        lst.add("org.jboss.eap:wildfly-ee-galleon-pack");
        lst.add("org.jboss.eap:wildfly-galleon-pack");
        return lst;
    }
    // End XXX DEMO
